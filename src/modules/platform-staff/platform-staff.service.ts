...
export class PlatformStaffService {
  ...
  async replaceRoles(
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      await tx.platformMemberRole.deleteMany({
        where: { platformMemberId: id },
      });
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      if (dto.status !== 'ACTIVE') {
        await tx.authSession.updateMany({
          ...
          data: {
            revokedAt: new Date(),
            revokeReason: `PLATFORM_${dto.status}`,
          },
        });
      }
      await tx.auditLog.create({
  ...
  private async resolveRoles(rawCodes: string[], actor: AuthenticatedUser) {
    const codes = [
      ...new Set(rawCodes.map((code) => code.trim().toUpperCase())),
    ];
    if (!codes.length)
  ...
  private async assertAnotherActiveSuperAdmin(excludedMemberId: string) {
    ...
    if (count < 1)
      throw new BadRequestException('The last active SUPER_ADMIN is protected');
  }
