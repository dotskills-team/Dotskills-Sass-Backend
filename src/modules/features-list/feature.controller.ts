...
export class FeatureController {
  ...
  // CREATE
  @Post()
  @RequirePlatformPermissions(PLATFORM_PERMISSIONS.FEATURE_CREATE)
  ...
  // GET ALL
  @Get()
  @RequirePlatformPermissions(PLATFORM_PERMISSIONS.FEATURE_READ)
  ...
  // UPDATE
  @Patch(':id')
  @RequirePlatformPermissions(PLATFORM_PERMISSIONS.FEATURE_UPDATE)
  update(
  ...
  // UPDATE STATUS
  @Patch(':id/status')
  @RequirePlatformPermissions(PLATFORM_PERMISSIONS.FEATURE_STATUS)
  updateStatus(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: UpdateFeatureStatusDto,
    @Req() req: any,
  ) {
    return this.featureService.updateStatus(id, dto, {
      actorUserId: req.user.userId,
      ipAddress: req.ip,
      userAgent: req.headers['user-agent'],
      requestId: req.headers['x-request-id'],
  ...
  // ACTIVATE
  @Patch(':id/activate')
  @RequirePlatformPermissions(PLATFORM_PERMISSIONS.FEATURE_ACTIVATE)
  activate(@Param('id', ParseUUIDPipe) id: string, @Req() req: any) {
    return this.featureService.activate(id, {
      actorUserId: req.user.userId,
      ipAddress: req.ip,
      userAgent: req.headers['user-agent'],
      requestId: req.headers['x-request-id'],
  ...
  // DEACTIVATE
  @Patch(':id/deactivate')
  @RequirePlatformPermissions(PLATFORM_PERMISSIONS.FEATURE_DEACTIVATE)
  deactivate(@Param('id', ParseUUIDPipe) id: string, @Req() req: any) {
    return this.featureService.deactivate(id, {
      actorUserId: req.user.userId,
      ipAddress: req.ip,
      userAgent: req.headers['user-agent'],
      requestId: req.headers['x-request-id'],
  ...
  // ARCHIVE
  @Patch(':id/archive')
  @RequirePlatformPermissions(PLATFORM_PERMISSIONS.FEATURE_ARCHIVE)
  archive(@Param('id', ParseUUIDPipe) id: string, @Req() req: any) {
    return this.featureService.archive(id, {
      actorUserId: req.user.userId,
      ipAddress: req.ip,
      userAgent: req.headers['user-agent'],
      requestId: req.headers['x-request-id'],
