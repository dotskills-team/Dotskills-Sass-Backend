...
import { AuditActorType, FeatureStatus, PlanStatus } from 'src/generated/phase-1-prisma/enums';
...
export class PlanFeatureService {
   ...
   */
  private async getModifiablePlan(
    planId: string,
  ) {
    const plan =
  ...
  private async getFeature(
    ...
    if (
      ...
      throw new BadRequestException(
        `Feature "${feature.code}" is not active`,
      );
    }
  ...
  async findAll(
    ...
    const planFeatures =
      await this.prisma.planFeature.findMany(
        {
          where: {
            planId,
  ...
  async findOne(
    ...
    if (!planFeature) {
      throw new NotFoundException(
        'Plan feature assignment not found',
      );
    }
