...
describe('StorageService', () => {
  ...
  describe('uploadFile', () => {
    it('uploads a base64 data URI under an extension-stripped public_id and returns the secure_url', async () => {
      (cloudinary.uploader.upload as jest.Mock).mockResolvedValue({
        secure_url:
          'https://res.cloudinary.com/test-cloud/image/upload/v1/company-logos/company-1/abc.png',
      });

      const buffer = Buffer.from('fake-image-bytes');
      const url = await service.uploadFile(
        buffer,
        'company-logos/company-1/abc.png',
        'image/png',
      );
      ...
      expect(cloudinary.uploader.upload).toHaveBeenCalledWith(
        ...
        {
          public_id: 'company-logos/company-1/abc',
          resource_type: 'image',
          overwrite: true,
        },
      );
    ...
    it('throws a clear error when a required credential is missing, without ever calling the SDK', async () => {
      ...
      await expect(
        service.uploadFile(Buffer.from('x'), 'k.png', 'image/png'),
  ...
  describe('deleteFile', () => {
    ...
    it('is idempotent — a "not found" result does not throw', async () => {
      ...
      await expect(
        service.deleteFile('missing-key.png'),
    ...
    it('throws when Cloudinary reports any other result', async () => {
      ...
      await expect(service.deleteFile('some-key.png')).rejects.toThrow(
        'Cloudinary destroy returned "error"',
      );
    });
  });
});
