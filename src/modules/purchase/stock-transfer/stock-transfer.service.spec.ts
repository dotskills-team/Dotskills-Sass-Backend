...
import { StockMovementType, StockTransferStatus } from '../../../generated/phase-1-prisma/enums';
...
describe('StockTransferService', () => {
  ...
  describe('create', () => {
    it('rejects when fromLocationId equals toLocationId', async () => {
      await expect(
        service.create(context, { fromLocationId: 'loc-1', toLocationId: 'loc-1', productId: 'p-1', quantity: 5 } as any, actor),
    ...
    it('creates a PENDING transfer without touching Inventory at all', async () => {
      mockTx.stockTransfer.create.mockResolvedValue({ id: 't-1', status: StockTransferStatus.PENDING });

      await service.create(context, { fromLocationId: 'loc-1', toLocationId: 'loc-2', productId: 'p-1', quantity: 5 } as any, actor);
  ...
  describe('dispatch', () => {
    const pending = { id: 't-1', fromLocationId: 'loc-1', toLocationId: 'loc-2', productId: 'p-1', quantity: 5, status: StockTransferStatus.PENDING };

    it('rejects dispatching a transfer that is not PENDING', async () => {
      mockPrisma.stockTransfer.findFirst.mockResolvedValue({ ...pending, status: StockTransferStatus.IN_TRANSIT });

      await expect(service.dispatch(context, 't-1', actor)).rejects.toThrow(BadRequestException);
      expect(mockInventoryService.decreaseStock).not.toHaveBeenCalled();
    });
    ...
    it('decreases stock at the source location with allowNegative:false and movementType TRANSFER_OUT, then moves to IN_TRANSIT', async () => {
      ...
      mockTx.stockTransfer.update.mockResolvedValue({ id: 't-1', status: StockTransferStatus.IN_TRANSIT });
      ...
      expect(mockTx.stockTransfer.update).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ status: StockTransferStatus.IN_TRANSIT }) }),
      );
  ...
  describe('receive', () => {
    const inTransit = { id: 't-1', fromLocationId: 'loc-1', toLocationId: 'loc-2', productId: 'p-1', quantity: 5, status: StockTransferStatus.IN_TRANSIT };

    it('rejects receiving a transfer that is not IN_TRANSIT', async () => {
      mockPrisma.stockTransfer.findFirst.mockResolvedValue({ ...inTransit, status: StockTransferStatus.PENDING });

      await expect(service.receive(context, 't-1', actor)).rejects.toThrow(BadRequestException);
      expect(mockInventoryService.increaseStock).not.toHaveBeenCalled();
    });
    ...
    it('increases stock at the destination location with movementType TRANSFER_IN, then moves to RECEIVED', async () => {
      ...
      mockTx.stockTransfer.update.mockResolvedValue({ id: 't-1', status: StockTransferStatus.RECEIVED });

      await service.receive(context, 't-1', actor);

      expect(mockInventoryService.increaseStock).toHaveBeenCalledWith(
        mockTx,
        expect.objectContaining({ productId: 'p-1', locationId: 'loc-2', quantity: 5, movementType: StockMovementType.TRANSFER_IN }),
      );
      expect(mockTx.stockTransfer.update).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ status: StockTransferStatus.RECEIVED }) }),
      );
