...
describe('Business Ops Sales/POS — multi-tenant isolation (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    const companyA = await companyManagementService.create(
      { tenantId: TENANT_1, industryId: INDUSTRY_ID, code: 'SALEISOA', legalName: 'Sales Isolation A (disposable)', baseCurrencyCode: 'BDT' } as any,
      CREATOR_USER_ID,
    );
    const companyB = await companyManagementService.create(
      { tenantId: TENANT_2, industryId: INDUSTRY_ID, code: 'SALEISOB', legalName: 'Sales Isolation B (disposable)', baseCurrencyCode: 'BDT' } as any,
      CREATOR_USER_ID,
    );
  ...
  it('gives both companies their own independent sale-number sequence, and stock for identical SKUs never bleeds across companies', async () => {
    const [locA, unitA, custA] = await Promise.all([
      locationService.create(contextA, { name: 'Counter', locationType: 'BRANCH' as any }, actor),
      unitService.create(contextA, { name: 'Piece', code: 'PCS' }, actor),
      customerService.create(contextA, { name: 'Customer A', phone: '01700000001' }, actor),
    ]);
    const productA = await productService.create(contextA, { sku: 'SHARED-SALE-SKU', name: 'Shared Sale SKU', baseUnitId: unitA.data.id, salePrice: 50, costPrice: 30 }, actor);

    const [locB, unitB, custB] = await Promise.all([
      locationService.create(contextB, { name: 'Counter', locationType: 'BRANCH' as any }, actor),
      unitService.create(contextB, { name: 'Piece', code: 'PCS' }, actor),
      customerService.create(contextB, { name: 'Customer B', phone: '01700000001' }, actor),
    ]);
    const productB = await productService.create(contextB, { sku: 'SHARED-SALE-SKU', name: 'Shared Sale SKU', baseUnitId: unitB.data.id, salePrice: 50, costPrice: 30 }, actor);
    ...
    await expect(
      saleService.create(
        contextB,
        { locationId: locB.data.id, items: [{ productId: productB.data.id, quantity: 1 }], payments: [{ method: SalePaymentMethod.CASH, amount: 50 }] } as any,
        actor,
      ),
    ).rejects.toThrow();
  ...
  it("Company A cannot read, void, or return against Company B's Sale by id", async () => {
    const locB = await locationService.create(contextB, { name: 'Cross-Read-Target', locationType: 'BRANCH' as any }, actor);
    const unitB = await unitService.create(contextB, { name: 'Box', code: 'BOX' }, actor);
    const productB = await productService.create(contextB, { sku: 'CROSS-SALE-READ', name: 'Cross Sale Read Product', baseUnitId: unitB.data.id, salePrice: 20, costPrice: 10 }, actor);
    await prisma.$transaction((tx) =>
      inventoryService.increaseStock(tx, { tenantId: TENANT_2, companyId: companyBId, productId: productB.data.id, locationId: locB.data.id, quantity: 5, movementType: StockMovementType.PURCHASE }),
    );
    const saleB = await saleService.create(
      contextB,
      { locationId: locB.data.id, items: [{ productId: productB.data.id, quantity: 1 }], payments: [{ method: SalePaymentMethod.CASH, amount: 20 }] } as any,
      actor,
    );

    await expect(saleService.findOne(contextA, saleB.data.id)).rejects.toThrow(NotFoundException);
    await expect(saleService.void(contextA, saleB.data.id, { reason: 'x' } as any, actor)).rejects.toThrow(NotFoundException);
    await expect(
      saleService.createReturn(contextA, saleB.data.id, { reason: 'x', items: [{ productId: productB.data.id, quantity: 1 }] } as any, actor),
  ...
  it("a customer payment for Company A's customer cannot be recorded against Company B's context, and never affects Company B's customer balances", async () => {
    const custA = await customerService.create(contextA, { name: 'Payment Isolation Customer A' }, actor);
    const custB = await customerService.create(contextB, { name: 'Payment Isolation Customer B' }, actor);

    await expect(
      customerPaymentService.recordPayment(contextB, { customerId: custA.data.id, amount: 100 } as any, actor),
    ).rejects.toThrow(NotFoundException);

    const stillA = await prisma.customer.findUniqueOrThrow({ where: { id: custA.data.id }, select: { dueBalance: true } });
    const stillB = await prisma.customer.findUniqueOrThrow({ where: { id: custB.data.id }, select: { dueBalance: true } });
